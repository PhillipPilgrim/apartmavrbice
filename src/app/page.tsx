  ...
  return (
    <div className={`${PoppinsRegular.className} text-black`}>
      ...
      >
        ...
        >
          <p
            ...
            } text-center text-2xl text-[#e94629] md:text-3xl lg:text-4xl`}
          >
            {t("about.title")}
          </p>
          ...
          <p
            className={`${
              PoppinsSemiBold.className
            } mt-4 text-lg text-[#e94629] md:text-xl lg:text-2xl`}
          >
            {t("about.offersTitle")}
          </p>
          <motion.ul
          ...
          >
            {(t("about.offers", { returnObjects: true }) as string[]).map(
          ...
          >
            ...
            <ul className="mt-2 space-y-1 text-justify text-[14px] md:text-[15px] lg:text-base">
              {(t("about.reasons", { returnObjects: true }) as string[]).map(
                (item: string, idx: number) => (
                  <li key={idx} className="flex gap-2">
        ...
        <motion.div
          ...
          viewport={{ once: true }}
          className="relative mt-12 flex flex-col ml-4 items-center justify-center gap-4 xl:flex-col"
        >
          <Image
          ...
          <Image
            ...
            width={1000}
            height={1000}
            className="h-full w-fit md:ml-10 shadow-lg md:w-md"
            priority
          />
        </motion.div>
      </section>
      ...
      <section className="mt-24 w-full gap-24 px-4 pb-36 md:px-12 lg:px-24">
        <p
          className={`${PoppinsSemiBold.className} text-center text-2xl md:text-3xl`}
        >
          {t("activities.title")}
        </p>

        <div className="mt-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {(t("activities.items", { returnObjects: true }) as { title: string; description: string }[]).map(
            (item: { title: string; description: string }, idx: number) => (
              ...
              >
                <div className="relative h-48 w-full">
                  <Image
                    src={`/assets/grids/${idx === 0 ? "castle" : idx === 1 ? "cyklo" : idx === 2 ? "gastro" : idx === 3 ? "walking" : idx === 4 ? "shopping" : "sport"}.jpg`}
                    alt="Banner"
                    layout="fill"
                ...
                <div className="p-6">
                  <h3 className={`${PoppinsSemiBold.className} mb-2 text-xl`}>
                    {item.title}
                  </h3>
